<template>
  <div tabindex="0"
       :class="{ 'active': isOpen, 'multiselect--disabled': disabled, 'multiple' : multiple }"
       @focus="activate()"
    ...
    <a v-for="option of visibleValue"
    ...
    <div class="default text" v-show="search.length == 0 && currentOptionLabel.length == 0" v-text="placeholder"></div>
    <span class="sizer" ref="sizer" v-text="search"></span>
    <input name="search"
           ref="search"
           type="text"
           autocomplete="off"
           ...
           class="multiselect_menu search"/>
    <div v-if="!multiple" v-show="search.length == 0" class="text" v-text="currentOptionLabel">
    </div>
    <span
    ...
    <transition name="multiselect">
      <div
        ...
        <div v-show="filteredOptions.length === 0 && search" class="message">
    ...
    <i @mousedown.prevent="toggle()" class="dropdown icon"></i>
  </div>
</template>
...
<style>
  .ui.dropdown .multiselect_menu.menu {
    display: block;
  }

  .ui.search.dropdown > input.multiselect_menu.search {
    width: inherit;
    min-width: 2.2em;
  }
